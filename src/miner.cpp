...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("InsuranceMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("InsuranceMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("InsuranceMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("insurance-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running InsuranceMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
